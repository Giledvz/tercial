...
- ☑ `hoja-de-respuestas-128` — la diseñó la IA de diseño como componente OMR propio (opción "B" repensada): `examenes-pdf/hoja-respuestas-128.html`. Además se creó **nueva** `hoja-respuestas-120.html` (universidad UNAM/IPN, 120 reactivos). HECHO 2026-06-30.
...
- 2026-06-30 · Bloque 16 (hoja de respuestas) cerrado vía la **IA de diseño**. Entregó dos hojas OMR en lenguaje Tercial (diseño "B" repensado): `hoja-respuestas-128.html` (prepa COMIPEMS, versiones 1–8) y `hoja-respuestas-120.html` (universidad UNAM/IPN, 120 reactivos, versiones 1–4) — esta última **nueva**, sin fuente `.tex`. Componente local `.hr-*` (óvalos con letra fantasma, agrupación por bloques, marcas de registro OMR, header en casillas, versión en burbuja); `--neutral` registrado en `exam-print.css`. Verificadas a 1 hoja Letter c/u. Enlazadas en `aula.html` (Materiales de clase, 20→22). Falta del bloque: `plantilla-simulacion`. El paquete completo de diseño (dashboard, mapa, niveles, portada/preguntas PDF, Telegram) vino en el zip "Portada para página de resultados" para Fase 1.
- 2026-06-15 · Claves de los exámenes semanales 2-8 y 13 (162 reactivos). Se migraron en su día solo en versión alumno porque el original de Overleaf no traía respuestas; ahora cada uno lleva clave del profesor con el mismo mecanismo `?clave=1` de las hojas (`.clave-only` + `.answer-final`, carga de `exam-clave.js`). Resueltos desde cero con verificación doble (solucionador independiente por examen + relectura directa de cada gráfica SVG y de las figuras geométricas). Convención: `g = 10 m/s²` (Gil la usa en clase para operar sin calculadora; coincide con examen-12-fisica). Hallazgos: los ángulos entre paralelas del banco son IGUALES, no suplementarios (confirmado en gen_bloque13: la variante 75°/4x+15 solo da entero si son iguales). Enlazados en el plegable de claves del aula. Las gráficas posición-tiempo del banco dan velocidades poco realistas (50-125 m/s) por ser auto-generadas; las respuestas son correctas para la gráfica dibujada (avisado a Gil).
